-- A valid public team invite should not return success while leaving an
-- existing inactive membership unusable. Re-activate public invite roles on
-- conflict, while preserving active coach/admin privileges.
...
BEGIN
  ...
  ON CONFLICT (team_id, user_id) DO UPDATE
    SET
      role = CASE
        WHEN team_members.status = 'inactive'::player_status THEN EXCLUDED.role
        WHEN team_members.role IN ('player'::team_role, 'parent'::team_role) THEN EXCLUDED.role
        ELSE team_members.role
      END,
      status = 'active'::player_status,
      updated_at = NOW()
    WHERE team_members.status = 'inactive'::player_status
      OR team_members.role IN ('player'::team_role, 'parent'::team_role);
