...
export function generateSessionPrintHTML(
  ...
  const appName = options.appName?.trim() || 'Session Planner';
  ...
  const activitiesHtml = session.activities
    .map((activity, index) => {
      ...
      const safePlayName = escapeHtml(activity.linked_play_name_snapshot);
      const safePlayThumbnail = escapeAttribute(activity.linked_play_thumbnail_data_url);
      ...
      const linkedPlayHtml = activity.linked_play_id
        ? `
          <div class="play-preview">
            ${
              safePlayThumbnail
                ? `<img src="${safePlayThumbnail}" alt="${escapeAttribute(activity.linked_play_name_snapshot || 'Linked play')}" />`
                : '<span class="play-preview-placeholder"></span>'
            }
            <div class="play-preview-meta">
              <p class="play-preview-name">${safePlayName || 'Linked Play'}</p>
              ${
                activity.linked_play_version_snapshot
                  ? `<p class="play-preview-version">v${activity.linked_play_version_snapshot}</p>`
                  : ''
              }
            </div>
          </div>
...
<body>
  ...
  <div class="footer">
    <div>Generated by ${escapeHtml(appName)}</div>
